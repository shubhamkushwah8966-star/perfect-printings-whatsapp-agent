...
async function replyToCustomer(to, text) {
  ...
  if (isFirstMessage && isGreetingOnly) {
    ...
    const ai = await fetch("https://api.openai.com/v1/responses", { method: "POST", headers: { Authorization: `Bearer ${OPENAI_API_KEY}`, "Content-Type": "application/json" }, body: JSON.stringify({ model: "gpt-5-mini", instructions, input: history.slice(-40).join("\n") }) });
...
http.createServer((req, res) => {
  ...
  req.on("end", () => {
    ...
    try {
      ...
      const incomingText = message?.text?.body || (message?.image ? `[Customer uploaded an image${message.image.caption ? `: ${message.image.caption}` : ""}]` : message?.document ? `[Customer uploaded a PDF/document${message.document.caption ? `: ${message.document.caption}` : ""}]` : "");
